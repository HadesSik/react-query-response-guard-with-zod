...
interface IFUpgradeReactQueryProps {
  className?: string
}

const UpgradeReactQuery: FC<IFUpgradeReactQueryProps> = ({ className }) => {
  const { data: correctData } = useSuspenseQueryFn<TPostUser1CorrectResponse>(
    'GET',
    {
      queryKey: ['upgrade', 'correct', POST_USER_1_CORRECT],
      endPoint: POST_USER_1_CORRECT,
    }
  )
  ...
  const { data: wrongData } = useSuspenseQueryFn<TPostUser1WrongResponse>(
    'GET',
    {
      queryKey: ['upgrade', 'wrong', POST_USER_1_CORRECT],
      endPoint: POST_USER_1_CORRECT,
    }
  )
  ...
  return (
    <div className={className} style={{ border: '1px solid #ddd' }}>
      <h2>UpgradeReactQuery [LEVEL 2]</h2>
      <hr />
      <div>
...
export default UpgradeReactQuery
